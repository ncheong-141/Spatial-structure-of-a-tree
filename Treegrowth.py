...
CV.Nb = 100        # Estimate number of branches/Memory increase of spatial arrays
...
# ------------------- Define functions from polycall file -------------------------------------

# Functions for different spatial plances (vy_x = vary branch coordinates in y on the xy plane)
...
# Initiate extrusion counter
k = 0

# Initiate trunk at origin and generate random values for control points
...
# Initiate trunk start position based on trunk height and % start position set 
k = round(CV.start_B*np.size(Sk[2,:,0]))

# Establish random index to skip portions of tree trunk
...
# condition to extend matrix (Python uses dynamic arrays)
WV.ext = 1
...
while k < np.size(Sk[2,:,0]):
    ...
    for i in range(0,WV.Nb_ds):
        ...
        """
        Coordinate transformation of branches
        So far generalized polynomail coordinates of each branch have been created 
        """
        # Establish random angle of each branch (radians) 
        WV.B_angle = rand.uniform(0,6.28) 
        
        # Copy of original X coords to remove data dependency (inefficient...)
        B_xcoords = copy.deepcopy(Sk[0,:,ko])
        
        # X coordinate transformation 
        # y = x'sin(theta) + y'cos(thets) + Trunk radius*cos(theta) + Origin of trunk segement,Ox
        Sk[0,:,ko] = Sk[0,:,ko]*np.cos(WV.B_angle) - Sk[1,:,ko]*np.sin(WV.B_angle) + (Tr[k]*np.cos(WV.B_angle) + Sk[0,k,0])
        
        # Y coordinate transformation 
        # y = x'sin(theta) + y'cos(thets) + Trunk radius*sin(theta) + Origin of trunk segement,Oy
        Sk[1,:,ko] = B_xcoords*np.sin(WV.B_angle) + Sk[1,:,ko]*np.cos(WV.B_angle) + (Tr[k]*np.sin(WV.B_angle) + Sk[1,k,0])
        
        # Z coordinate transformation 
        # z = z + Origin of trunk segment,Oz
        Sk[2,:,ko] = Sk[2,:,ko] + Sk[2,k,0]
